//Pico_CrossPoint controller using mini blue solderless breadboards with MCP4822 DAC
// 12/10/2025
...
void setup() {
  ...
  pinMode(led, OUTPUT);
  pinMode(0, OUTPUT); // CSA strobe for switch chip A 
  pinMode(1, OUTPUT); // CSB strobe for switch chip B 
  pinMode(2, OUTPUT); // CSC strobe for switch chip C
  pinMode(3, OUTPUT); // CSD strobe for switch chip D 
  pinMode(4, OUTPUT); // CSE strobe for switch chip E 
  pinMode(5, OUTPUT); // common Clock input
  pinMode(6, OUTPUT); // common Reset input
  pinMode(7, OUTPUT); // common Data input
  pinMode(8, OUTPUT); 
  pinMode(9, OUTPUT); 
  pinMode(10, OUTPUT); 
  pinMode(11, OUTPUT); 
  ...
  digitalWrite(5, HIGH); // CLK input idles High
  delayMicroseconds(6); // Wait
  digitalWrite(6, HIGH); // Toggel Reset pin High
  delayMicroseconds(6); // Wait
  digitalWrite(6, LOW); // Reset input defaults Low
  digitalWrite(7, LOW); // Data input defaults Low
  digitalWrite(0, LOW); // Chip A Strobe input defaults Low
  digitalWrite(1, LOW); // Chip B Strobe input defaults Low
  digitalWrite(2, LOW); // Chip C Strobe input defaults Low
    ...
    if (Serial.available()){
      ...
      switch (c){
        case 'I': // read back FW Rev
          Serial.println ("Pi Pico Cross Point Mini Blue");
          break;
        ...
        case 'x': // Reset all cross point switches to off
          digitalWrite(6, HIGH); // Toggel Reset pin High
          delayMicroseconds(6); // Wait
          digitalWrite(6, LOW); // Toggel Reset pin Low
          break;
          //
        case 'X': // set cross point switch at address x , y, on chip () to on / off
          ...
          // Send first data bit
          if(xadr & 0b100) {
            digitalWrite(7, HIGH); // Data is High
          }else{
            digitalWrite(7, LOW); // Data is Low
          }
          delayMicroseconds(1); // Wait
          digitalWrite(5, LOW); // CLK Low
          delayMicroseconds(5); // Wait
          digitalWrite(5, HIGH); // CLK High
          delayMicroseconds(1); // Wait
          ...
          if(swon == 1){
            digitalWrite(7, HIGH); // Data is High switch on
          }else{
            digitalWrite(7, LOW); // Data is Low switch off
          }
          //
          delayMicroseconds(4); // Wait
          digitalWrite(cadr-1, HIGH); // STB High
          delayMicroseconds(5); // Wait
          digitalWrite(cadr-1, LOW); // STB Low
          delayMicroseconds(5); // Wait
          digitalWrite(7, LOW); // Data is Low switch off
